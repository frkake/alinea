...
- 生成・再生成・ブロック書き直しは ``jobs(kind='article')`` へ委譲する(実行は
  :mod:`yakudoku_worker.tasks.generate_article`。202 を返し、クライアントは
...
- ``content`` の wire 形変換(DB のフラット保存形 → plans/03 §19.1 のネスト形)は
  :mod:`yakudoku_core.article.wire` に委譲する(worker の ``jobs.result`` と同じ変換)。
"""
...
from yakudoku_core.article.wire import build_article_block_wire
...
from yakudoku_api.routers.papers import StorageDep
from yakudoku_api.routers.viewer import resolve_owned_library_item
from yakudoku_api.schemas.articles import (
    ArticleBlockOut,
...
from yakudoku_api.schemas.viewer import asset_url

router = APIRouter(tags=["articles"])
log = structlog.get_logger("yakudoku.api.articles")
