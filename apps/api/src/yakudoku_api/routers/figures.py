...
- 生成(書き直し・単体再生成)は ``jobs(kind='figure')`` へ委譲する(実行は
  :mod:`yakudoku_worker.tasks.generate_overview_figure` /
  :mod:`yakudoku_worker.tasks.generate_explainer_figure`。202 を返し、クライアントは
...
from yakudoku_api.schemas.figures import (
...
log = structlog.get_logger("yakudoku.api.figures")
...
async def _download_filename(db: AsyncSession, article: Article, version: int) -> str:
    ...
    return f"yakudoku-overview-{slug or 'paper'}-v{version}.svg"
