...
``ALINEA_KEY_ENCRYPTION_SECRET``(Fernet 標準 44 文字 urlsafe base64)。カンマ区切りで
...
from alinea_llm.protocols import ResolvedKey
...
class DbKeyStore:
    ...
    def _cipher(self) -> MultiFernet:
        if self._fernet is None:
            self._fernet = _build_fernet(self._settings.alinea_key_encryption_secret)
        return self._fernet
