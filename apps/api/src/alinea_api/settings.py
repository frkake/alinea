...
共有設定(DB / Redis / S3 / セッション秘密鍵など)は `alinea_core.settings.CoreSettings`
...
class ApiSettings(CoreSettings):
    ...
    smtp_username: str = ""
    smtp_password: str = ""
    smtp_from: str = "login@alinea.app"
    ...
    # (worker も .env から読めるようにするため。alinea_core.settings 参照)。

    # ルート解決の Redis キャッシュ TTL(秒。plans/04 §15・§16。既定 60)。
    alinea_llm_route_cache_ttl_s: int = 60
    ...
    def allowed_origins(self) -> list[str]:
        ...
        - 常に app_base_url(dev: http://localhost:3000 / prod: https://alinea.app)。
