...
入口(plans/00 §9・plans/12 §14.1 の逐語):
    ...
    python -m alinea_api.seed --sample rectified-flow [--reset] [--scale N] [--full]

Rectified Flow(arXiv:2209.03003)を全テスト・VR・開発の共通データ源として投入する。
フィクスチャは ``seed_data/rectified_flow/``(JSON + assets)。ブロック安定 ID は
``alinea_core.parsing.block_ids.assign_block_ids`` で決定的に導出し、
``block_search_index`` は ``alinea_core.search.rebuild`` で再構築する。

投入エンティティ: users(``dev@alinea.test``)/ papers / source_assets /
...
from alinea_core.document.plaintext import chat_content_to_plain
...
DEV_EMAIL = "dev@alinea.test"
MEMBER_EMAIL = "member@alinea.test"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m alinea_api.seed", description="開発シード投入"
    )
