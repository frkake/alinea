...
from alinea_api.schemas.common import LastPosition, LibraryItemSummary, PaperBib
...
def _reference_citation_display(record: dict[str, Any], idx: int) -> str:
    structured = _structured_dict(record.get("structured"))
...
def _inline_to_wire(inline: Any) -> dict[str, Any]:
    if hasattr(inline, "model_dump"):
        dumped = inline.model_dump(mode="json", exclude_none=True)
...
async def list_references(revision_id: str, user: CurrentUser, db: DbDep) -> ReferencesResponse:
    ...
    for entry in entries:
        structured = _structured_dict(entry.get("structured"))
