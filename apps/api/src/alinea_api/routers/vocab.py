...
- ``POST /api/vocab`` は保存直後に 201 を返し、AI 生成(8 フィールド。plans/07 §7)は
  ``jobs(kind='vocab')`` へ委譲する(interactive キュー。:mod:`alinea_worker.tasks.
...
- SRS は :mod:`alinea_api.services.srs_service`(固定段階方式。docs/11 §7.1)に委譲する。
- 出典表示(``source.display`` / ``anchor.display``)は ``block_search_index`` から
  :mod:`alinea_api.chat.evidence` 経由で導出する(注釈・メモと同方式)。
"""
...
import structlog
from alinea_core.db.models import LibraryItem, Paper, VocabEntry
...
from alinea_api.errors import PROBLEM_CONTENT_TYPE, ProblemException, build_problem
...
from alinea_api.services.srs_service import apply_review, next_review_display

router = APIRouter(tags=["vocab"])
log = structlog.get_logger("alinea.api.vocab")
...
async def export_vocab_markdown(
    ...
    filename = f"alinea-vocab-{today.strftime('%Y%m%d')}.md"
    return Response(
        content=content,
