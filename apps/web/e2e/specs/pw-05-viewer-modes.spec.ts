...
/**
 * PW-05(plans/12 §4.3・M0 スコープ): ビューア基本。
 * M0 は表示 3 モード(訳文 / 対訳 / 原文)のワンクリック切替(plans/13 §1.5)。品質バッジ常時、
 * サイドパネル 3 タブ(チャット / 図表 / 情報)の排他。PDF / 記事モード・図表参照ポップは
 * M1/M2 スコープ or コンテンツ依存のため test.fixme(下記)。
 */
test.describe("PW-05 ビューア基本(3 モード・タブ排他)", () => {
  ...
  test("サイドパネルは 3 タブで排他選択される", async ({ page }) => {
    ...
    // メモ・注釈・リソースタブは M0 では非表示。
    await expect(page.getByRole("tab", { name: "メモ" })).toHaveCount(0);
    await expect(page.getByRole("tab", { name: "注釈" })).toHaveCount(0);
