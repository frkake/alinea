...
export const articleKeys = {
  all: (liId: string) => ["article", liId] as const,
  article: (liId: string, preset: Preset | null = "beginner") => ["article", liId, preset] as const,
 ...
 */
export async function fetchArticle(itemId: string, preset: Preset | null = null): Promise<Article> {
  const res = await articlesGet({
    path: { item_id: itemId },
    query: preset ? { preset } : undefined,
    throwOnError: true,
  });
  return res.data;
}
