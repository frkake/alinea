...
``apps/api/src/alinea_api/routers/resources.py`` と意図的に重複させる
(``alinea_worker.bootstrap`` が SSE envelope 形式を複製する先例と同方針)。
...
``apps/worker/src/alinea_worker/tasks/__init__.py``(共有ファイル)側の変更が必要なため
followups に委ねる。
"""
