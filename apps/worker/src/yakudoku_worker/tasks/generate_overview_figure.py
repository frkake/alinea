...
- 初回生成(v1)は記事初回・再生成ジョブ(:mod:`yakudoku_worker.tasks.generate_article`)の
...
は :mod:`yakudoku_figures.overview_svg`(決定的・バイト同一)に委譲する。
"""
...
from yakudoku_core.article import ArticleSources, EvidenceDisplayResolver, build_evidence_wire
...
from yakudoku_llm.types import ContentPart, ImageRequest, LLMRequest, LLMResponse, Message
from yakudoku_llm.types import JsonSchemaSpec as _JsonSchemaSpec

log = structlog.get_logger("yakudoku.worker")

JST = dt.timezone(dt.timedelta(hours=9))
MAX_GENERATION_ATTEMPTS = 3  # 初回 + 再試行 2 回(plans/07 §5.2「数値照合エラー」)
FOOTER_GENERATED_BY = "✦ AI 生成 · 訳読"
...
def build_overview_raster_prompt(headings_en: tuple[str, str, str]) -> str:
    ...
    from yakudoku_worker.tasks.generate_explainer_figure import EXPLAINER_STYLE_PREAMBLE
