...
> 対象読者と前提: 本書は「訳読 / YAKUDOKU — 論文読解ワークベンチ」のフロントエンド実装者向けに、画面 1d(ダッシュボード)を確定デザインと 100% 一致させるための完全仕様である。機能仕様は docs/06 §6(ダッシュボード)・docs/02(取り込みパイプライン)を正とし、ピクセル値は抽出ファイル extract/1d.md の値を逐語で採用する(本書 §4 に全量転記済み)。共通コンポーネント名は plans/08-design-system.md、API エンドポイント名は plans/03-api.md、データ型は plans/02-data-model.md に従う。技術スタック: Next.js 15(App Router)+ React 19 + TypeScript 5 + Tailwind CSS v4 + TanStack Query v5 + Zustand。
...
import type { LibraryItemSummary, Status } from '@yakudoku/api-client';
...
│ [訳ロゴ+訳読+YAKUDOKU w=198][検索バー w=460]……[◷通知][YKアバター]         │
├──────────┬────────────────────────────────────────────────────────────────┤
...
1. ロゴブロック ◇ AppLogo(flex / align-items:center / gap:8px / width:198px。クリックで `/` へ):
   - 「訳」マーク: inline-flex中央寄せ、22×22px、border-radius:6px、背景 var(--pr-a,#3E5C76)、文字色 #FFFFFF、font-size:11.5px、font-weight:700。
   - 「訳読」: font-size:14.5px、font-weight:700、letter-spacing:0.5px。
   - 「YAKUDOKU」: font-size:9.5px、色 #B0B4BA、letter-spacing:1.2px、margin-top:2px。
...
トップバー: 「訳」「訳読」「YAKUDOKU」「ライブラリ全体を検索 — 本文・訳文・メモ・チャット」「⌘K」「◷」「YK(デモ)」
...
- スケルトン共通片: `div.yk-skeleton` — 背景 var(--pr-bg-muted,#EFEDE6)、border-radius:4px、アニメーション `yk-pulse 1.2s ease-in-out infinite alternate`(opacity 0.55 → 1.0)。
- 形状(実カードと同寸の枠内に配置):
...
- 解除の永続化: Zustand ストア `useDashboardUiStore`(persist ミドルウェア、name: `yk-dashboard-ui` — これが localStorage の実キー名)のフィールド `queueWarnDismissedCount: number | null` に「× を押した時点の本数」を保存する(独立した localStorage キーは作らない。決定)。表示判定は `count >= 6 && count !== queueWarnDismissedCount`。本数が変化(7 本に増える・6 本に戻る等)したら再表示される。
...
- [ ] キュー 5 本以下で警告バナー非表示、6 本以上で表示。「×」で消え、リロード後も再表示されない(localStorage キー `yk-dashboard-ui` 内の `queueWarnDismissedCount`)。本数が変わると再表示。「整理する」で `/library?status=up_next` へ遷移
