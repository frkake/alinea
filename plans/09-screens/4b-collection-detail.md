...
- 共有 URL 表示: `share.url.replace(/^https?:\/\//, '')` → `yakudoku.app/c/x8Kf3qPw`。
...
│ [訳ロゴ+訳読 w=198] [検索バー 460×32] …(flex:1)… [◷ 30×30] [YK 30×30]     │
├───────────┬───────────────────────────────────────────────────────────────┤
...
1. ロゴブロック(flex, align-items:center, gap:8px, width:198px)
   - ロゴマーク: 「訳」1 文字。inline-flex 中央、22×22px、border-radius:6px、背景 `var(--pr-a)`(既定 #3E5C76)、文字 #FFFFFF、font-size:11.5px、font-weight:700。
...
- URL 行(flex, align-items:center, gap:6px, border 1px solid #E7E4DB(`--pr-border-pane`), border-radius:6px, padding:6px 9px, 背景 #FBFAF7(`--pr-bg-app`)):
  - URL「yakudoku.app/c/x8Kf3qPw」font-family `'IBM Plex Mono', monospace`、font-size:10.5px、#3C4046(`--pr-text-mid`)、flex:1、white-space:nowrap、overflow:hidden、text-overflow:ellipsis。
...
共有リンクカード: 「閲覧用共有リンク」「発行済み」「yakudoku.app/c/x8Kf3qPw」「コピー」「閲覧のみ · アカウント不要 · noindex」「共有ページにメモを含める」「1 件」「共有ページを確認 →」「リンクを無効化」
...
`collectionKeys.detail` 解決前はシェル(トップバー・サイドバー)を即時描画し、メインエリアにスケルトンを表示する。スケルトン片は背景 #EFEDE6(`--pr-bg-muted`)、border-radius:4px、`animation: yk-pulse 1.2s ease-in-out infinite`(opacity 1→0.55→1。tokens 隣接 ui 層 CSS に定義):
...
- [ ] 共有: 未発行→「共有リンクを発行」で active になり 8 文字トークン URL が表示される。「コピー」でクリップボードに `https://yakudoku.app/c/{token}` が入り文言が 2 秒「コピーしました」になる。トグルで `PATCH …/share` が飛び `included_note_count` 表示が維持される。「リンクを無効化」は確認モーダル経由で `DELETE …/share`、revoked 描画+再発行導線になる。再発行すると**新しい**トークンになる。
