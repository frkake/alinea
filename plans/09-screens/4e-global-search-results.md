...
└─ LibraryShell                                  共有シェル(1e §1)
   ...
   └─ SearchResultsColumn                        固有
      ...
      │   └─ SearchHitRow × n                    固有
      │       ├─ SourceBadge(§5.22)              共通
      │       ├─ (スニペット。<mark class="alinea-search-hit">)
      │       └─ (メタ行+ジャンプリンク)
...
│ [A|Alinea w=198] [検索ボックス w=460 フォーカス状態] …[◷][YK]  │
├───────────┬──────────────────────────────────────────────────┤
...
1. **ロゴブロック**(flex、align-items:center、gap:8px、width:198px)
   - アプリマーク「A」: inline-flex 中央、22×22px、border-radius:6px、`background: var(--pr-acc)`(#3E5C76)、color:#FFFFFF、font-size:11.5px、font-weight:700。
...
- 本文カラム: 縦 flex、gap:3px、min-width:0、flex:1。
  ...
  - **ハイライト `<mark class="alinea-search-hit">`**(plans/08 §5.17 の決定): `background: rgba(196,148,50,0.30)`(琥珀 30%)、border-radius:2px、padding:0 1px、color:inherit。
...
※ [ ] 内が `<mark class="alinea-search-hit">` ハイライト部。訳文側は対訳語「EMA 教師(EMA teacher)」全体が 1 つの mark。
...
- トップバー: 「A」「Alinea」「EMA teacher」(クエリ)「×」「◷」「YK」
...
- 全体に `animation: alinea-pulse 1.2s ease-in-out infinite`(opacity 1→0.55→1。1e §5.2 と同一 keyframes)。
...
- `document.title`(決定): `「{q}」の検索結果 — Alinea`。未入力時は `検索 — Alinea`。
...
- [ ] `<mark class="alinea-search-hit">` が rgba(196,148,50,0.30)・radius 2px・padding 0 1px。訳文行では「EMA 教師(EMA teacher)」全体が 1 マーク。
...
1. **plans/03 §15.1**: レスポンスに以下を追加する(API 実装前に反映必須):
   ...
   - `snippet` のコメント `<mark class="hit">` を **`<mark class="alinea-search-hit">` に修正**する(クラス名は plans/08 §5.17 の決定 `.alinea-search-hit` が正。サーバーはこのクラス名で `<mark>` を出力する)。
