...
| M1-18 | PDF パーサ(品質 B)+受け口 | PyMuPDF+pdfplumber によるテキスト・bbox 抽出、段組み・読み順復元、見出し検出、図領域切り出し、書誌推定、`POST /api/ingest/pdf`(private 既定・50MB/415・本文不足時は worker の OCR 最終候補) | `alinea_core/parsing/pdf_parser.py` `ingest/bib_estimate.py` plans/03 §3.3 実装、テスト PY-PARSE-03, PY-ING-04 | L | M0-41 | plans/05 §6, §9; docs/02 §4 |
...
| ID | タスク名 | 内容 | 主な成果物 | 規模 | 依存 | 参照 |
|---|---|---|---|---|---|---|
| M2-01 | LaTeX パーサ | arXiv e-print(LaTeX ソース)→ 品質 A 構造化、相互参照解決(`\ref`/`\cite`)、取得優先順位を LaTeX > HTML > PDF に切替(主経路化) | `alinea_core/parsing/latex_parser.py`(parser_version 'latex-1.3.0')fetch 優先順位変更、テスト PY-PARSE-02 | L | M1-25 | plans/05 §5, §1.3; docs/02 §3 |
...
| M3-02 | OCR(品質 B 内部拡張・**前倒し完了 2026-07-11**) | スキャン/本文不足 PDF のテキスト化。品質レベルは新設せず B の内部拡張。原本・埋め込み・アップロード共通の text→OCR 候補、隔離実行、readiness、versioned checkpoint identity を M1-18 hardening で実装済み | `pdf_parser.py` / `source_candidates.py` の OCR 分岐、API/worker readiness、plans/05 §6 追補、OCR 回帰テスト | L | M1-18 | plans/05 §6.1; docs/10 §5; docs/02 §4 |
