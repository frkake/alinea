...
> 対象読者と前提: 「訳読 / YAKUDOKU — 論文読解ワークベンチ」を docs/00〜12(機能仕様の正)と確定デザイン16画面に100%忠実に実装する開発者。本書はリポジトリ構成・開発環境・ツールチェーン・規約・CI を確定させる基盤文書であり、以降の plans 全文書はここで定義した識別子(ディレクトリ・環境変数・コマンド)を前提とする。技術選定の正は spec-decisions C 項。機能仕様と矛盾した場合は docs が正。
...
リポジトリルートは `paper-reader/`(GitHub リポジトリ名 `yakudoku`)。pnpm workspaces + Turborepo(JS)と uv workspace(Python)を同居させる。
...
│   │   ├── package.json           # name: "@yakudoku/web"
│   │   ├── next.config.ts
...
│   │       └── styles/globals.css # @import "@yakudoku/tokens/css"; @theme マッピング
│   │
│   ├── api/                       # FastAPI
│   │   ├── pyproject.toml         # name: "yakudoku-api"。依存に yakudoku-core(workspace)
│   │   ├── alembic.ini
│   │   ├── alembic/
│   │   │   ├── env.py
...
│   │
│   ├── worker/                    # arq ワーカー
│   │   ├── pyproject.toml         # name: "yakudoku-worker"。依存に yakudoku-core(workspace)
│   │   └── src/yakudoku_worker/
...
│           │   └── arxiv-pill.content.ts  # arXiv 限定「訳 保存」ピル(既定オフ、docs/08 §5)
│           ├── lib/api.ts         # packages/api-client を利用
│           └── lib/pdf-detect.ts  # タブ内 PDF 判定・書誌ローカル推定
│
└── packages/
    ├── tokens/                    # デザイントークン(単一ソース)
    │   ├── package.json           # name: "@yakudoku/tokens"。exports: {"./css": "./dist/tokens.css", "./js": "./dist/tokens.js"}
    ...
    │   ├── package.json           # name: "@yakudoku/api-client"
    ...
    │   ├── pyproject.toml         # name: "yakudoku-core"
    │   └── src/yakudoku_core/
    ...
    └── llm/                       # LLM / 画像プロバイダ抽象化層(完全定義は plans/04 §2)
        ├── pyproject.toml         # name: "yakudoku-llm"(import 名 yakudoku_llm)
        ├── models.yaml            # モデルレジストリのシード(plans/04 §7)
        ├── routing.yaml           # タスクルーティングのシード(plans/04 §8)
        └── src/yakudoku_llm/      # types.py, protocols.py, registry.py, routing.py, router.py, providers/(openai_provider.py, anthropic_provider.py, google_provider.py, deepseek_provider.py, xai_provider.py, images/)
```

構成上の決定:

- 決定: Python 共有コードは `packages/py-core`(パッケージ名 `yakudoku_core`)に一本化し、apps/api と apps/worker は同パッケージに依存する。理由: モデル・パーサ・翻訳層を api/worker で二重定義すると必ず乖離するため。uv workspace のパス依存で編集は即時反映される。
- 決定: LLM / 画像プロバイダ抽象化層のみは `packages/llm`(パッケージ名 `yakudoku_llm`)に分離する(plans/04 §2 と一致)。apps/api・apps/worker の双方が path 依存で参照する。理由: 依存(各社 SDK・tiktoken)とリリースサイクルが py-core と異なり、plans/04 のテストスイートを独立実行するため。
...
services:
  db:
    ...
    container_name: yakudoku-db
    ports:
      - "5432:5432"
    environment:
      POSTGRES_USER: yakudoku
      POSTGRES_PASSWORD: yakudoku
      POSTGRES_DB: yakudoku
      POSTGRES_INITDB_ARGS: "--locale=C --encoding=UTF8"
    volumes:
      - db-data:/var/lib/postgresql/data
      - ./docker/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U yakudoku -d yakudoku"]
      interval: 5s
      timeout: 3s
      retries: 10
  ...
  minio:
    ...
    container_name: yakudoku-minio
    ports:
      - "9000:9000"   # S3 API
      - "9001:9001"   # 管理コンソール
    environment:
      MINIO_ROOT_USER: yakudoku
      MINIO_ROOT_PASSWORD: yakudoku-dev-secret
  ...
  minio-init:
    ...
    entrypoint: >
      /bin/sh -c "
      mc alias set local http://minio:9000 yakudoku yakudoku-dev-secret &&
      mc mb --ignore-existing local/yakudoku-sources &&
      mc mb --ignore-existing local/yakudoku-assets &&
      exit 0
      "

  mailpit:
    image: axllent/mailpit:v1.27.0
    container_name: yakudoku-mailpit
    ports:
      - "1025:1025"   # SMTP
...
- 決定: バケットは `yakudoku-sources`(SourceAsset 原本: LaTeX ソース tar・PDF 原本)と `yakudoku-assets`(派生物: 図画像・サムネイル・概要図 SVG/DSL スナップショット・解説図ラスター)の2つ。理由: 原本(削除不可・再処理原資、docs/09 §2)と派生物(再生成可能)でライフサイクル・バックアップポリシーを分けるため。
...
| Playwright | 1.53.2 | apps/web(E2E) | `pnpm --filter @yakudoku/web e2e` |
...
S3_BUCKET_SOURCES=yakudoku-sources
S3_BUCKET_ASSETS=yakudoku-assets
...
- 決定: 秘匿値の読み込みは apps/api・apps/worker とも `yakudoku_api/settings.py` と同形の pydantic-settings クラス(`yakudoku_core.settings.CoreSettings`)経由に限定し、`os.environ` 直接参照を Ruff の禁止ルール(カスタム flake8-tidy-imports 相当の `banned-api`)で防ぐ。
...
- 依存方向は `routers → services → (yakudoku_core の db / document …、および yakudoku_llm)` の一方向のみ。routers から SQLAlchemy モデルを直接返さない(必ず schemas の Pydantic に詰め替える)。
- worker の tasks は services を呼ばず yakudoku_core を直接使う(api の services は HTTP 文脈を含むため)。ジョブ投入は api 側 `services/jobs.py` の 1 モジュールに集約。
- 例外は yakudoku_core.errors の型付き例外(`SourceFetchError`, `PlaceholderMismatchError` …)を投げ、routers 側の exception handler で HTTP ステータスへ変換する(黙って壊れない: 失敗理由はジョブテーブルの `failure_reason` に構造化保存)。

### 6.3 import ルール

- TS: 相対 import は同一ディレクトリ(`./`)と 1 階層下のみ許可。親方向(`../`)は禁止し、`@/` エイリアスを使う(ESLint `no-restricted-imports` で強制)。apps 間の import は禁止。packages からの import は公開 exports(`@yakudoku/tokens/js` 等)のみ。
- Python: import は絶対パス(`from yakudoku_core.document import anchor`)のみ。相対 import 禁止(Ruff `TID252` 相当を有効化)。apps/api から apps/worker への import(逆も)は禁止 — 共有物は必ず packages/py-core(LLM 層は packages/llm)へ。
...
jobs:
  ...
  python:
    ...
    services:
      db:
        ...
        ports: ["5432:5432"]
        options: >-
          --health-cmd "pg_isready -U yakudoku" --health-interval 5s
    ...
    steps:
      ...
      - run: uv run mypy .
      - run: psql "postgresql://yakudoku:yakudoku@localhost:5432/yakudoku" -c "CREATE EXTENSION IF NOT EXISTS pgroonga; CREATE EXTENSION IF NOT EXISTS pgcrypto;"
      - run: uv run alembic upgrade head
        working-directory: apps/api
      - run: uv run pytest --cov=yakudoku_core --cov=yakudoku_llm --cov=yakudoku_api --cov=yakudoku_worker
  ...
  openapi-drift:
    ...
    steps:
      ...
      - run: uv sync --all-packages --frozen
      - run: pnpm install --frozen-lockfile
      - run: uv run python -m yakudoku_api.export_openapi > packages/api-client/openapi.json
      - run: pnpm --filter @yakudoku/api-client generate
  ...
  e2e:
    ...
    steps:
      ...
      - run: uv run alembic upgrade head
        working-directory: apps/api
      - run: uv run python -m yakudoku_api.seed --sample rectified-flow   # C10 シード(arXiv:2209.03003)
      - run: pnpm --filter @yakudoku/web exec playwright install --with-deps chromium
      - run: pnpm --filter @yakudoku/web e2e     # webServer 設定で api(uvicorn)+worker(arq)+web を起動
        env:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          ...
          cache: pnpm
      - run: pnpm install --frozen-lockfile
      - run: pnpm --filter @yakudoku/extension zip          # Chrome 用
      - run: pnpm --filter @yakudoku/extension zip:edge     # Edge 用(同一コード、名前のみ)
      - uses: actions/upload-artifact@v4
        with:
          name: extension-zips
...
- 決定: `yakudoku_api.export_openapi` モジュール(FastAPI アプリから openapi.json を標準出力する 5 行のスクリプト)を api 側に置き、クライアント生成の入力を常にコードから導出する(サーバー起動不要)。
...
```bash
# 1. 取得と環境変数
git clone git@github.com:<org>/yakudoku.git && cd yakudoku
...
pnpm turbo build --filter=@yakudoku/tokens --filter=@yakudoku/api-client

# 7. 開発サーバー一括起動(turbo が並列実行)
pnpm dev
#   ├─ @yakudoku/web        → http://localhost:3000(Next.js dev)
#   ├─ yakudoku-api         → http://localhost:8000(uvicorn --reload。OpenAPI UI は /docs)
#   ├─ yakudoku-worker      → arq ワーカー(watchfiles で自動再起動)
#   └─ @yakudoku/extension  → wxt dev(Chrome を拡張ロード済みで起動)

# 8. 動作確認
#   - http://localhost:3000/login → メールアドレス入力 → http://localhost:8025 でリンクを開く
#   - wxt が開いた Chrome で https://arxiv.org/abs/2209.03003 を開き、ツールバー「訳」→保存
#   - MinIO コンソール http://localhost:9001(yakudoku / yakudoku-dev-secret)
```

`pnpm dev` の実体はルート package.json の `"dev": "turbo dev"` で、turbo.json の `dev` タスク(`persistent: true, cache: false`)が各 app の `dev` スクリプトを並列起動する。Python 側の dev スクリプトは package.json を持つ薄いラッパ(`apps/api/package.json` の `"dev": "uv run uvicorn yakudoku_api.main:app --reload --port 8000"`、`apps/worker/package.json` の `"dev": "uv run arq yakudoku_worker.main.WorkerSettings --watch src"`)として turbo に参加させる。
