...
- 本番: `https://yakudoku.app/api/*`(apps/web の Next.js から同一オリジン。`next.config.ts` の `rewrites` で FastAPI へプロキシ)。拡張機能は `https://yakudoku.app/api/*` を直接呼ぶ。
- 開発: web=`http://localhost:3000` → rewrite → api=`http://localhost:8000`。
- 共有ページの公開 URL `https://yakudoku.app/c/{token}` は apps/web のページであり、データは §14 の匿名 API から取得する。
...
拡張は第一候補としてセッションクッキー共有(`host_permissions: https://yakudoku.app/*` + `credentials: "include"`)を使い、Safari 等でクッキーが使えない将来環境のフォールバックとして拡張トークンを使う(docs/08 §1)。
...
- `type` は `https://yakudoku.app/problems/{code のケバブケース}` で決定的に導出。`errors[]` は 422 のときのみ `[{ "field": "body.deadline", "message": "日付形式(YYYY-MM-DD)ではありません" }]` 形式。
...
4. Turborepo: ルート `turbo.json` にタスク `codegen` を定義(`inputs: ["apps/api/app/**"]`, `outputs: ["packages/api-client/openapi.json", "packages/api-client/src/schema.d.ts"]`)。`apps/web#build` と `apps/extension#build` は `dependsOn: ["@yakudoku/api-client#codegen"]`。
...
```
- 動作: 32 バイト URL-safe トークン(**有効 15 分・単回**)を発行し、`https://yakudoku.app/api/auth/email/verify?token=…` をメール送信。
...
```json
{ "type": "https://yakudoku.app/problems/duplicate", "title": "既にライブラリにあります",
...
Response 200: { items: {
  ...
  snippet: string;                            // <mark class="yk-search-hit"> 付き HTML エスケープ済み
}[] }
```
...
event: error
data: {"type":"https://yakudoku.app/problems/provider-error","title":"回答の生成に失敗しました","status":502,"code":"provider_error"}
```
...
- クエリ: §11.1 と同じフィルタ → **200** `text/markdown; charset=utf-8` + `Content-Disposition: attachment; filename="yakudoku-vocab-20260706.md"`。
...
GET(詳細) Response 200: {
  ...
  share: { status: "none" | "active" | "revoked";
           token: string | null; url: string | null;   // "https://yakudoku.app/c/x8Kf3qPw"
...
type SearchHit = {
  ...
  snippet: string;                   // <mark class="yk-search-hit">検索語</mark> 付き(サニタイズ済み HTML)
...
type OverviewFigureDsl = {           // 構造化図データ(SVG 決定的レンダリングの入力)
  ...
  footer: { generated_by: string; date: string };  // 「✦ AI 生成 · 訳読 · 2026-07-06」
};
...
```
- SVG は同一 DSL から**バイト同一**で再生成できる決定的レンダラ(実装は plans/07 §5.4 の `yakudoku_figures.overview_svg`)。SVG 内にフッタ(AI 生成表記)を含む。
...
event: error
data: {"type":"https://yakudoku.app/problems/ingest-failed","title":"取り込みに失敗しました","status":502,"code":"ingest_failed","detail":"stage=parsing: テキストが抽出できません","retryable":true}
```
