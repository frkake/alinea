...
> 対象読者と前提
> 本書は「Alinea — 論文読解ワークベンチ」の翻訳パイプライン(自然訳/直訳の生成・再翻訳・用語集・共有キャッシュ・品質検査・進捗)の実装計画である。対象読者は apps/worker・apps/api・packages/py-core の実装者。機能仕様の正は [docs/03-translation.md](../docs/03-translation.md)(および docs/01 §4–6・docs/02 §5)。データモデルは [plans/02-data-model.md](02-data-model.md)、API は [plans/03-api.md](03-api.md) §7、ジョブ基盤は [plans/01-architecture.md](01-architecture.md) §3.2・§4、LLM 抽象化層は [plans/04-llm-providers.md](04-llm-providers.md) を正とし、本書はそれらの識別子をそのまま使う(再定義しない)。基盤計画に不足する項目は §16「⚠ 基盤への追加要求」に集約した。
...
| コンポーネント | 責務 | 配置 |
|---|---|---|
| プレースホルダ codec | インライン保護のエンコード/デコード/検証(§4) | `packages/py-core/src/alinea_core/translation/placeholder.py` |
| プロンプトビルダ | system/user メッセージ構築(§5–6)・対訳例集(§5.4) | `packages/py-core/src/alinea_core/translation/prompts.py` / `examples.py` |
| 用語集マージ | 3層マージ・スナップショット・逆引き検索(§8) | `packages/py-core/src/alinea_core/translation/glossary.py` |
| 品質検査 | 5種の機械チェック(§12) | `packages/py-core/src/alinea_core/translation/quality.py` |
| パイプライン駆動 | バッチ化・LLM 呼び出し・再試行・UPSERT(§3–7) | `packages/py-core/src/alinea_core/translation/pipeline.py` |
| arq タスク | `translate_section` 等のジョブ実装 | `apps/worker/src/alinea_worker/tasks/translate_blocks.py` |
| API | plans/03 §7 のエンドポイント群 | `apps/api/src/alinea_api/routers/translations.py` + `services/translation_service.py` |
...
3. `translating_body` 段で残りセクションぶんの `translation` ジョブ(`payload.reason='initial'`。§3.1)をセクション文書順に `alinea:bulk` へ張り出す([plans/05](05-ingest-pipeline.md) §11.2)。
...
| 設定項目(4f 逐語) | 既定 | 実装 |
|---|---|---|
| 「付録(Appendix)を自動翻訳しない」 | ON | ON: 付録セクションの `translate_section` ジョブを積まない。OFF: 初回翻訳時に付録セクションも `alinea:bulk` に積む(**分母は変えない**。§13.1) |
...
|---|---|---|---|
| `translate_section` | `initial` / `literal` / `on_demand` / `table` | `alinea:bulk`(繰り上げ・オンデマンドは `alinea:interactive` + `priority=100`。plans/05 §2.5) | セクション単位の翻訳(初回・直訳・付録オンデマンド・表) |
| `retranslate_blocks` | `retranslate` / `instructed` / `glossary_change` | `alinea:interactive` | 再翻訳・指示つき再翻訳・用語変更の影響ブロック再翻訳 |
...
- ユーザーが開いているセクションの繰り上げ(§10.1)・直訳・付録・表・再翻訳は常に `alinea:interactive`([plans/01](01-architecture.md) §3.2 の決定)。
...
```python
# packages/py-core/src/alinea_core/translation/placeholder.py
...
3. §8.3 で影響ブロックを検索し、`retranslate_blocks` ジョブ(`reason='glossary_change'`、`alinea:interactive`)を 1 本 enqueue(`block_ids` 一括)。**モデルは通常の `translation` タスクルート**を使う(決定。エスカレーション(§11)は「訳の品質を疑った」場合の脱出口であり、用語置換起因の一括再翻訳は物量作業のため。docs/03 §9 のエスカレーション対象は再翻訳・指示つき再翻訳のみ)。
...
該当行があれば同一 `job_id` を `alinea:interactive` に**二重 enqueue**する(先着 claim が勝ち、後着は no-op。plans/05 §2.3)。該当行なし(実行中・完了・対象外)は 202 のまま no-op。連続スクロールでの多重呼び出しも無害(繰り上げ済みジョブの priority がさらに上がるだけ)だが、デバウンスはクライアント側 1,000ms(plans/05 §2.5)。
...
2. なければセット作成(public → shared / private → personal。§9.2)+ 親ジョブ(`jobs.kind='translation'`・`payload.reason='literal'`(セット全体。§3.1)、`alinea:interactive`、冪等キー `xlateset:{set_id}`)を作成し **202** `{set_id, job_id}`。
3. 親ジョブは §2.1 のスコープでセクション単位の子ジョブ群(`kind='translation'`・`reason='literal'`・`payload.section_id` あり)を作る。`priority_section_id` のセクションを `alinea:interactive` + `priority=100`、残りをセクション順で `alinea:bulk` に積む(docs/03 §5 の決定: 表示中セクション優先、以降は自然訳と同じキュー)。親ジョブは子の完了集計で `jobs.progress`(plans/02 §4.13)を進め、全完了で `complete`。
...
`POST /api/translation-sets/{set_id}/sections/{section_id}/translate`(plans/03 §7.5)→ `translate_section`(`reason='on_demand'`、`alinea:interactive`)。目次の「付録 A 証明 — 未翻訳」「開くと翻訳します(オンデマンド)」ボックス(1a)からは、付録を開いた時点でクライアントがこの API を呼ぶ。付録ブロックは §2.1 のスコープ外なので、`block_ids` はスコープ判定を**付録除外なし**で再計算した「そのセクションの翻訳可能ブロック」(type 条件のみ適用)。進捗の分母・分子には入らない(§13.1)。
...
`POST /api/translation-units/{unit_id}/retranslate {instruction?, discard_edit?}`(plans/03 §7.6)→ `retranslate_blocks`(`reason='retranslate' | 'instructed'`、block 1 件、`alinea:interactive`)。
...
7. **LLM 層のパッケージ配置の不一致**: [plans/00](00-tech-stack.md) §2 は `packages/py-core/src/alinea_core/llm/`、[plans/04](04-llm-providers.md) §2 は `packages/llm/src/alinea_llm/`。本書は plans/04(`packages/llm` / `alinea_llm`)を正として参照する。plans/00 側の修正を要求する(翻訳ロジック(`alinea_core.translation.*`)の配置は plans/00 のとおり)。
...
- [ ] 「開いたセクションを優先翻訳」で対象 translate_section が `alinea:interactive` に繰り上がり、二重実行が起きない(claim 検証)
