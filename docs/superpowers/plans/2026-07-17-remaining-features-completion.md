...
**Goal:** 論文プレゼンテーション／PPTX生成を除き、監査で判明したデータ欠損、不通導線、部分実装機能を製品として完了させる。

**Architecture:** 最初にデータ保全と既存導線の欠陥を修正し、その後に既存APIだけがある機能のUIを接続する。
続いて非同期エクスポート、ユーザー別LLMルーティング、他サイト取り込み、セマンティック検索、オフライン閲覧、記事公開を独立した縦スライスとして実装する。
各スライスは機能フラグまたは既存挙動へのフォールバックを持ち、単独でテスト、コミット、ロールバックできるようにする。

**Tech Stack:** Python 3.12、FastAPI、SQLAlchemy 2、Alembic、PostgreSQL 16、PGroonga、pgvector、ARQ、Redis、S3互換ストレージ、Next.js App Router、React、TanStack Query、TypeScript、Vitest、Playwright、PyMuPDF、KaTeX。
...
| 監査項目 | 実装タスク |
|---|---|
| 完全バックアップの欠損、上書き、UI誤表示 | 1〜4、15、18、21、22 |
...
| API契約、文書、DB分離、E2E残件 | 27、28 |
| 論文プレゼンテーション／PPTX生成 | 対象外 |

## 1. 実行順とマージ単位

### 実装依頼の起点

この文書を全機能実装の起点とし、Task 1からTask 28まで番号順に実行する。
...
論文プレゼンテーションおよびPPTX生成は別計画であり、この文書を実行するときは変更しない。

ユーザー受け入れ確認は、各フェーズで分割して実施しても、フェーズJの完了後にまとめて実施してもよい。
...
| J | 27〜28 | 全体回帰、OpenAPI一致、文書一致、E2Eが通る |
...
### Task 27: API契約、仕様書、生成クライアントを現在の機能へ揃える
...
公開機能、他サイト、Hugging Face関連ソース、GitHubコード対応解析、セマンティック検索、オフライン閲覧、Anki TSV、easy translationを実装済みの契約へ変更する。
PPTXはこの計画の対象外であり、別計画であることを明記する。
...
```bash
git add apps/api/src/alinea_api/routers/translations.py packages/api-client/openapi.json packages/api-client/src/generated docs/00-product.md docs/01-domain-model.md docs/02-ingest.md docs/03-translation.md docs/06-library.md docs/07-figures-and-articles.md docs/10-roadmap.md docs/11-vocabulary.md docs/12-resources.md docs/superpowers/plans/2026-07-16-all-features-integration-report.md
git commit -m "docs: align product contracts with completed features"
```

### Task 28: 全体回帰、DB分離、E2Eの未検証箇所を解消する
...
一般ユーザー二件、公開者一件、外部サイト別のサンプルURL、バックアップ用論文、GitHubコード対応の正解例を確認環境へ用意する。

- [ ] **Step 10: ユーザー受け入れ確認を依頼する**
...
- [ ] **Step 11: 判定結果に応じて処理する**

`FAIL` または `BLOCKED` の場合はmainへマージせず、該当タスクへ戻って修正、対象テスト、Task 28の全検証、最終コミット、リリース候補作成を繰り返す。
`GO` の場合は、確認後にコード、設定、生成物、文書を変更しない。

- [ ] **Step 12: 確認済みコミットだけをmainへ統合する**
...
フェーズJの実装作業は、Task 28の必須コマンドがすべて成功し、OpenAPI再生成で差分が出ず、統合レポートが実測結果と一致する最終コミットを作成した場合に完了とする。
...
- ユーザー所有の未追跡ファイル `.superpowers/brainstorm/` と `docs/superpowers/specs/2026-07-16-paper-presentation-tool-design.md` を変更またはコミットしない。
