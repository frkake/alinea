...
:class:`yakudoku_llm.types.JsonSchemaSpec` にそのまま渡す draft 2020-12 の JSON Schema。
``ArticleV1Model`` / ``ArticleBlockModel`` はモデル出力(検証済み JSON)を型付きで扱うための
Pydantic モデルで、JSON Schema 検証の後段(型アクセス用)として使う。

**モデル出力のフィールド名**(このモジュールの型)と**DB 保存形**(:mod:`yakudoku_core.article.
postprocess` が書く ``article_blocks.content``)は異なる(後者は
:func:`yakudoku_core.document.plaintext.article_block_to_plain` が要求するフラットな
