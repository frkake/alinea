...
- :mod:`yakudoku_core.article.schema`: 記事構造 JSON スキーマ(モデル出力)。
- :mod:`yakudoku_core.article.sources`: 素材収集(stage=collecting_sources)。
- :mod:`yakudoku_core.article.prompts`: 生成プロンプト(system/user、プリセット 4 種)。
- :mod:`yakudoku_core.article.postprocess`: 検証・正規化(stage=generating の後半)。

ジョブ実行(DB 書き込み・LLM 呼び出し)は :mod:`yakudoku_worker.tasks.generate_article` の責務。
"""
...
from yakudoku_core.article.prompts import (
...
from yakudoku_core.article.storage_keys import (
