"""ジョブ回復コマンド: `python -m yakudoku_core.jobs.requeue`(plans/01 §4.1)。

Redis が消失しても DB の `status='queued'` 行は残る。本コマンドはそれらを arq(Redis)へ
再 enqueue し、完全復旧させる。arq に渡すのは job_id のみ(ペイロードは DB が保持)。

**逸脱メモ**: 本モジュールは redis/arq を使うが、これらは yakudoku-core の宣言依存ではない
...
QUEUE_FOR_KIND: dict[str, str] = {
    "ingest": "yk:bulk",
    "translation": "yk:bulk",
    "article": "yk:interactive",
    "figure": "yk:interactive",
    "vocab": "yk:interactive",
    "resource_meta": "yk:interactive",
    "export": "yk:bulk",
    "account_delete": "yk:bulk",
}
DEFAULT_QUEUE = "yk:bulk"
