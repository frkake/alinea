...
出力は `yakudoku_core.parsing.html_parser.ParsedDocument`(既存 IR を再利用。重複定義しない)
...
from yakudoku_core.document.blocks import Block, Section, SectionHeading
from yakudoku_core.document.inlines import Inline
from yakudoku_core.parsing.block_ids import assign_block_ids
from yakudoku_core.parsing.html_parser import ParsedDocument
...
def _resolve_bibliography(text: str, files: dict[str, str]) -> str:
    """`\\bibliography{}` を同梱 `.bbl` の内容で置換する(plans/05 §5「参考文献 .bbl」)。"""
    ...
    if bbl_name is None:
        return text
    return text[: m.start()] + files[bbl_name] + text[m.end() :]
...
def _read_optional_braced(text: str, pos: int) -> tuple[str | None, int]:
    """コマンド直後の任意の `[...]` を読み飛ばし、続く `{...}` があれば内容を返す。"""
    i = pos
...
def _build_bibliography_blocks(inner: str) -> list[Block]:
    ...
    for idx, m in enumerate(matches):
        ...
        if not semi_raw:
            continue
        structured = _structure_reference(semi_raw)
        blocks.append(
            Block(
                id="",
...
class _LatexParser:
    ...
    def _make_section(self, nodes: list[tuple[Any, ...]], idx: int) -> tuple[Section, int]:
        ...
        sec.blocks.append(
            Block(id="", type="heading", level=level, number=number or None, title=title or None)
        )
        if label:
