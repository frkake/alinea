...
- `pgroonga_snippet_html` の 1 断片を最終 HTML(`<mark class="alinea-search-hit">`)へ整形(§5.1)
...
MARK_OPEN = '<mark class="alinea-search-hit">'
MARK_CLOSE = "</mark>"
...
def finalize_snippet_html(raw_fragment: str) -> str:
    ...
    - `<span class="keyword">` → `<mark class="alinea-search-hit">`、`</span>` → `</mark>`
