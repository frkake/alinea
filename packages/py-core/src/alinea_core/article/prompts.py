...
_ARTICLE_SYSTEM_TEMPLATE = (
    ...
    "7. explainer_figure(解説図)は最大 2 個。image_brief_en には論文固有の入力、"
    "処理段階、出力、比較条件、矢印の向き、各要素の配置を英語で具体的に書く。"
    "3〜7 個の構成要素で、何が新規性・因果関係なのか一目で分かる技術模式図にする。"
    "論文中に実在しない天秤、霧、雲、歯車などの装飾的な比喩は禁止する。"
    "短い英語ラベルは最大 5 個まで許可するが、文章・数式・根拠のない数値は含めない。"
    "caption_ja には図から読み取るべき内容を具体的に書く。\n"
