...
モデル出力(:mod:`alinea_core.article.schema` の ``ArticleV1Model``)を検証・正規化し、
DB 保存形(:mod:`alinea_core.document.plaintext` の ``article_block_to_plain`` が要求する
フラットなキー名)に変換する。ライセンス判定は :mod:`alinea_core.licenses` の既存マトリクスを
...
from alinea_core.article.schema import (
