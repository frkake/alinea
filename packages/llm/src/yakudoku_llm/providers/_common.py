...
ベース URL 上書き環境変数 YAKUDOKU_{PROVIDER}_BASE_URL(plans/12 §15 ⚠-2)に対応する。
...
def base_url_override(provider: str, default: str | None = None) -> str | None:
    """YAKUDOKU_{PROVIDER}_BASE_URL があれば優先、なければ default(SDK 既定は None)。"""
    return os.environ.get(f"YAKUDOKU_{provider.upper()}_BASE_URL") or default
